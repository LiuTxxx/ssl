...
class Trainer():
    def __init__(self, pwd):
        ...
        self.optimizer = torch.optim.SGD(param_list, lr=cfg.lr, weight_decay=cfg.weight_decay, momentum=cfg.momentum,
                                         nesterov=True)

        self.n_iters_per_epoch = cfg.n_imgs_per_epoch // cfg.batch_size

        self.lr_schdlr = WarmupCosineLrScheduler(self.optimizer, max_iter=self.n_iters_per_epoch * cfg.n_epoches,
                                                 warmup_iter=0)
        ...
        self.writer, self.log_txt = logger(cfg.exp_path, cfg.exp_name, pwd,
    ...
    def evaluate(self, ema):
        ...
        self.train_record = update_model(ema.model, self.optimizer, self.lr_schdlr, self.epoch, self.i_tb,
                                         self.exp_path, self.exp_name, acc, self.train_record)
