...
def loading_data():
    ...
    if cfg.dataset == 'CIFAR10':
        ...
        print(
            f"Unlabeled examples: {len(base_dataset.targets) - cfg.n_labeled}  after expand: {cfg.mu * cfg.n_imgs_per_epoch}")
        __, train_unlabeled_idxs = x_u_split(base_dataset.targets, cfg.n_labeled, num_classes=10)

        unlabeled_dataset = CIFAR10SSL(root, unlabeled_indexs=train_unlabeled_idxs, train=True,
                                       transform=TransformFix(32, mean_std))
    ...
    if cfg.dataset == 'CIFAR100':
        ...
        unlabeled_dataset = CIFAR100SSL(root, unlabeled_indexs=train_unlabeled_idxs, train=True,
                                        transform=TransformFix(32, mean_std))
        sampler_u = RandomSampler(unlabeled_dataset, replacement=True, num_samples=cfg.mu * cfg.n_imgs_per_epoch)
        batch_sampler_u = BatchSampler(sampler_u, cfg.mu * cfg.batch_size, drop_last=True)
        unlabeled_loader = DataLoader(unlabeled_dataset, batch_sampler=batch_sampler_u, num_workers=4, pin_memory=False)
    ...
    if cfg.dataset == 'STL10':
        ...
        print("Dataset: STL10")
        print(
            f"Unlabeled examples: {len(base_dataset.labels) - cfg.n_labeled}  after expand: {cfg.mu * cfg.n_imgs_per_epoch}")
        __, train_unlabeled_idxs = x_u_split(base_dataset.labels, cfg.n_labeled, num_classes=10)

        unlabeled_dataset = STL10SSL(root, unlabeled_indexs=train_unlabeled_idxs, split='train',
                                     transform=TransformFix(48, mean_std))
        sampler_u = RandomSampler(unlabeled_dataset, replacement=True, num_samples=cfg.mu * cfg.n_imgs_per_epoch)
        batch_sampler_u = BatchSampler(sampler_u, cfg.mu * cfg.batch_size, drop_last=True)
        unlabeled_loader = DataLoader(unlabeled_dataset, batch_sampler=batch_sampler_u, num_workers=4, pin_memory=False)
    ...
    if cfg.dataset == 'SVHN':
        ...
        unlabeled_dataset = SVHNSSL(root, unlabeled_indexs=train_unlabeled_idxs, split='train',
                                    transform=TransformFix(32, mean_std))
        sampler_u = RandomSampler(unlabeled_dataset, replacement=True, num_samples=cfg.mu * cfg.n_imgs_per_epoch)
        batch_sampler_u = BatchSampler(sampler_u, cfg.mu * cfg.batch_size, drop_last=True)
        unlabeled_loader = DataLoader(unlabeled_dataset, batch_sampler=batch_sampler_u, num_workers=4, pin_memory=False)
    ...
    if cfg.dataset == 'MNIST':
        ...
        unlabeled_dataset = MNISTSSL(root, unlabeled_indexs=train_unlabeled_idxs, train=True,
                                     transform=TransformFix(32, mean_std), download=True)
        sampler_u = RandomSampler(unlabeled_dataset, replacement=True, num_samples=cfg.mu * cfg.n_imgs_per_epoch)
        batch_sampler_u = BatchSampler(sampler_u, cfg.mu * cfg.batch_size, drop_last=True)
        unlabeled_loader = DataLoader(unlabeled_dataset, batch_sampler=batch_sampler_u, num_workers=4, pin_memory=False)
...
def update_loading(indexs=None, pre_targets=None, epoch=None):
    ...
    if cfg.dataset == 'CIFAR10':
        root = './dataset/CIFAR10'
        labeled_dataset = CIFAR10SSL(root, labeled_indexs=all_indexs, labeled_targets=all_targets, train=True,
                                     transform=TransformFix(32, mean_std))
    ...
    if cfg.dataset == 'CIFAR100':
        root = './dataset/CIFAR100'
        labeled_dataset = CIFAR100SSL(root, labeled_indexs=all_indexs, labeled_targets=all_targets, train=True,
                                      transform=TransformFix(32, mean_std))
        sampler_l = RandomSampler(labeled_dataset, replacement=True, num_samples=cfg.n_imgs_per_epoch)
        batch_sampler_l = BatchSampler(sampler_l, cfg.batch_size, drop_last=True)
        labeled_loader = DataLoader(labeled_dataset, batch_sampler=batch_sampler_l, num_workers=4, pin_memory=False)
    ...
    if cfg.dataset == 'STL10':
        root = './dataset/STL10'
        labeled_dataset = STL10SSL(root, labeled_indexs=all_indexs, labeled_targets=all_targets, split='train',
                                   transform=TransformFix(48, mean_std))
        sampler_l = RandomSampler(labeled_dataset, replacement=True, num_samples=cfg.n_imgs_per_epoch)
        batch_sampler_l = BatchSampler(sampler_l, cfg.batch_size, drop_last=True)
        labeled_loader = DataLoader(labeled_dataset, batch_sampler=batch_sampler_l, num_workers=4, pin_memory=False)
    ...
    if cfg.dataset == 'MNIST':
        root = './dataset/MNIST'
        labeled_dataset = MNISTSSL(root, labeled_indexs=all_indexs, labeled_targets=all_targets, train=True,
                                   transform=TransformFix(32, mean_std))
        sampler_l = RandomSampler(labeled_dataset, replacement=True, num_samples=cfg.n_imgs_per_epoch)
        batch_sampler_l = BatchSampler(sampler_l, cfg.batch_size, drop_last=True)
        labeled_loader = DataLoader(labeled_dataset, batch_sampler=batch_sampler_l, num_workers=4, pin_memory=False)
    ...
    if cfg.dataset == 'SVHN':
        root = './dataset/SVHN'
        labeled_dataset = SVHNSSL(root, labeled_indexs=all_indexs, labeled_targets=all_targets, split='train',
                                  transform=TransformFix(32, mean_std))
        sampler_l = RandomSampler(labeled_dataset, replacement=True, num_samples=cfg.n_imgs_per_epoch)
        batch_sampler_l = BatchSampler(sampler_l, cfg.batch_size, drop_last=True)
        labeled_loader = DataLoader(labeled_dataset, batch_sampler=batch_sampler_l, num_workers=4, pin_memory=False)
